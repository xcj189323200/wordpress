...
function charge_pay()
{
    ...
    if ($pay_type == 6) {
        ...
        $data=array(
            ...
            'title'     => $order_name, //必须的，订单标题，长度32或以内
            'time'      => time(),//必须的，当前时间戳，根据此字段判断订单请求是否已超时，防止第三方攻击服务器
            'notify_url'=>  get_stylesheet_directory_uri() . '/shop/xunhupay/notify2.php', //必须的，支付成功异步回调接口
            'return_url'=> get_stylesheet_directory_uri() . '/shop/xunhupay/return.php',//必须的，支付成功后的跳转地址
            'callback_url'=> esc_url(home_url('/user?action=charge')),//必须的，支付发起地址（未支付或支付失败，系统会会跳到这个地址让用户修改支付信息）
            'modal'=>null, //可空，支付模式 ，可选值( full:返回完整的支付网页; qrcode:返回二维码; 空值:返回支付跳转链接)
            'nonce_str' => str_shuffle(time())//必须的，随机字符串，作用：1.避免服务器缓存，2.防止安全密钥被猜测出来
    ...
    if ($pay_type == 7 || $pay_type == 8) {
        ...
        $params = array(
            ...
            "param" => "rimini",//自定义参数
            "notify_url"=>get_stylesheet_directory_uri() . '/shop/codepay/notify.php',//通知地址
        ); //构造需要传递的参数
        ...
        if ($resultData && $resultData['status'] == 0) {
            if ($paymethod == 3) {
                $iconstr = '<img src="'.get_stylesheet_directory_uri() . '/assets/icons/weixin.png" class="qr-pay">';
                echo json_encode(array('status' => '1', 'type' => '1', 'msg' => '微信扫码支付 '.$resultData['money'].' 元'.$iconstr, 'img' => $resultData['qrcode'], 'num' => $order_trade_no));
                exit;
            }else{
                $iconstr = '<img src="'.get_stylesheet_directory_uri() . '/assets/icons/alipay.png" class="qr-pay">';
                echo json_encode(array('status' => '1', 'type' => '1', 'msg' => '支付宝扫码支付 '.$resultData['money'].' 元'.$iconstr, 'img' => $resultData['qrcode'], 'num' => $order_trade_no));
                exit;
            }
