...
if (!function_exists('cao_widget_pay')) {
    function cao_widget_pay($args, $instance)
    {
        ...
        if (!is_user_logged_in()) {
            echo '<a class="login-btn btn btn--primary btn--block"><i class="fa fa-user"></i> 登录购买</a>';
        } elseif ($PostPay->isPayPost()) {
            echo '<a target="_blank" href="'.esc_url(home_url('/go?downid='.$post_id)).'" class="go-down btn btn--secondary btn--block"><i class="fa fa-cloud-download"></i> 下载资源</a>';
            if ($cao_pwd) {
            ...
            if ($cao_is_boosvip && is_boosvip_status($user_id)) {
                echo '<button type="button" class="click-pay btn btn--primary btn--block" data-postid="' . $post_id . '" data-nonce="' . $create_nonce . '" data-price="0' . $site_money_ua . '">使用永久'.$site_vip_name.'特权购买</button>';
            }else{
        ...
        if ($instance['is_desc_info']) {
            ...
            if ($cao_is_boosvip) {
                echo '<div class="boosvip-abs"><i class="fa fa-info-circle"></i> 该资源永久'.$site_vip_name.'免费 <a href="'.esc_url(home_url('/user?action=vip')).'" ><i class="fa fa-hand-o-right"></i> 去开通</a></div>';
            }
