...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<div class="single-breadcrumbs">
		<span>您的位置</span> <i class="fa fa-angle-right"></i> <a href="<?php echo home_url(); ?>">首页</a> <i class="fa fa-angle-right"></i> <?php zimeiti_2_first_category(); ?>
	</div>
			<?php endif; ?>
	...
	<div class="entry-content">
		<?php 
			if( ( get_theme_mod('single-featured-on', false) == true ) && has_post_thumbnail() && (strpos($post->post_content,'[gallery') === false) ): 
				the_post_thumbnail('zimeiti_2_single_thumb'); 
			endif;
		?>	
		...
		<?php
			the_content( sprintf(
				...
				wp_kses( __( '阅读全文 %s <span class="meta-nav">&rarr;</span>', 'zimeiti-2' ), array( 'span' => array( 'class' => array() ) ) ),
				the_title( '<span class="screen-reader-text">"', '"</span>', false )
			) );

			wp_link_pages( array(
				'before' => '<div class="page-links">' . esc_html__( '分页:', 'zimeiti-2' ),
				'after'  => '</div>',
			) );
		?>
	</div><!-- .entry-content -->
	...
	<div class="entry-footer clear">
		...
		<div class="entry-tags">
			...
			<?php if (has_tag()) { ?><span class="tag-links"><span><?php esc_html_e( '标签:', 'zimeiti-2' ); ?></span><?php the_tags(' ', ' '); ?></span><?php } ?>
				
			<?php
				edit_post_link(
					sprintf(
						/* translators: %s: Name of current post */
						esc_html__( '编辑 %s', 'zimeiti-2' ),
						the_title( '<span class="screen-reader-text">"', '"</span>', false )
					),
...
<div class="author-box clear">
	...
	<div class="author-meta">	
		<h4 class="author-name"><?php echo __('关于作者:', 'zimeiti-2'); ?> <span class="hover-underline"><a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) ); ?>"><?php echo get_the_author_meta('nickname'); ?></a></span></h4>	
...
<?php
	...
	$args = apply_filters( 'zimeiti_2_related_posts_args', $query );
...
<?php
	...
	// Allow dev to filter the query.
	$args = apply_filters( 'zimeiti_2_related_posts_args', $custom_query_args );
