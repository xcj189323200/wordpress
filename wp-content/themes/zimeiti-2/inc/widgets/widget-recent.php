 ...
 * @since      1.0.0
 */
class zimeiti_2_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-zimeiti_2-recent widget_posts_thumbnail',
			'description' => __( '显示最新的文章（带缩略图）', 'zimeiti-2' )
		);

		// Create the widget.
		parent::__construct(
			'zimeiti_2-recent',                                   // $this->id_base
			__( '&raquo; 最新的文章', 'zimeiti-2' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'zimeiti_2_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'zimeiti_2_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'zimeiti_2_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( '近期文章', 'zimeiti-2' ),
			'limit'     => 5,
			'show_date' => true
		);
