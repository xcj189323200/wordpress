 ...
 * @since      1.0.0
 */
class zimeiti_2_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-zimeiti_2-popular widget_posts_thumbnail',
			'description' => __( '显示评论最多的文章（带缩略图）', 'zimeiti-2' )
		);

		// Create the widget.
		parent::__construct(
			'zimeiti_2-popular',                                   // $this->id_base
			__( '&raquo; 评论最多的文章', 'zimeiti-2' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'zimeiti_2_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'zimeiti_2_popular_widget_' . $this->id, $popular );
					...
					echo '<li class="clear">';
						...
						if ( has_post_thumbnail() ) {
							...
							echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
								the_post_thumbnail('zimeiti_2_widget_post_thumb');  
							echo '</div>' . '</a>';
							
						} else {

							echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
							echo '<img src="'. get_template_directory_uri() . '/thumb.php?src=' . catch_that_image() . '&w=120&h=85" alt="<?php the_title(); ?>"/>';
							echo '</div>' . '</a>';
	...
	function flush_widget_transient() {
		delete_transient( 'zimeiti_2_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( '评论最多的文章', 'zimeiti-2' ),
			'limit'     => 5,
			'show_views' => true
		);
