...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( '点赞这篇文章', 'boke-x' ),
		'unlike' => __( '取消点赞', 'boke-x' )
	) ); 
}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( '取消点赞', 'boke-x' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( '点赞这篇文章', 'boke-x' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	...
	$count = '<span class="sl-count"><i class="fa fa-thumbs-o-up"></i> ' . $number . '<em>赞</em>' . '</span>';
	return $count;
} // get_like_count()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( '您点赞的文章:', 'boke-x' ); ?></label></th>
			<td>
			<?php
