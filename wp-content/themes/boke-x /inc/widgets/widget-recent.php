 ...
 * @since      1.0.0
 */
class boke_x_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-boke-x-recent widget_posts_thumbnail',
			'description' => __( '显示最新的文章', 'boke-x' )
		);

		// Create the widget.
		parent::__construct(
			'boke_x-recent',                                   // $this->id_base
			__( '&raquo; 最新的文章', 'boke-x' ), // $this->name
	...
	function flush_widget_transient() {
		delete_transient( 'boke_x_recent_widget_' . $this->id );
	}
