 ...
 * @since      1.0.0
 */
class boke_x_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-boke-x-popular widget_posts_thumbnail',
			'description' => __( '显示评论最多的文章', 'boke-x' )
		);

		// Create the widget.
		parent::__construct(
			'boke_x-popular',                                   // $this->id_base
			__( '&raquo; 评论最多的文章', 'boke-x' ), // $this->name
	...
	function flush_widget_transient() {
		delete_transient( 'boke_x_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( '评论最多的文章', 'boke-x' ),
			'limit'     => 6
		);
