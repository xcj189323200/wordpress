...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( '点赞这篇文章', 'm-site' ),
		'unlike' => __( '取消点赞', 'm-site' )
	) ); 
}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( '取消点赞', 'm-site' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( '点赞这篇文章', 'm-site' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	...
	$count = '<span class="sl-count"><i class="fa fa-thumbs-o-up"></i> ' . esc_html('点赞', 'm-site') . $number . '</span>';
	return $count;
} // get_like_count()
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'm-site' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'm-site' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
