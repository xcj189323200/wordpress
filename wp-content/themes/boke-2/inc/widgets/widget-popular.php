 ...
 * @since      1.0.0
 */
class boke_2_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-boke_2-popular widget_posts_thumbnail',
			'description' => __( '显示评论最多的文章（带缩略图）', 'boke-2' )
		);

		// Create the widget.
		parent::__construct(
			'boke_2-popular',                                   // $this->id_base
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'boke_2_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'boke_2_popular_widget_' . $this->id, $popular );
					...
					echo '<li class="clear">';
						if ( has_post_thumbnail() ) {
							...
							echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
								the_post_thumbnail('boke_2_widget_post_thumb');  
							echo '</div>' . '</a>';
	...
	function flush_widget_transient() {
		delete_transient( 'boke_2_popular_widget_' . $this->id );
	}
