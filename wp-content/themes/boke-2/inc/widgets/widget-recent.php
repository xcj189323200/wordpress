 ...
 * @since      1.0.0
 */
class boke_2_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-boke_2-recent widget_posts_thumbnail',
			'description' => __( '显示最新的文章（带缩略图）', 'boke-2' )
		);

		// Create the widget.
		parent::__construct(
			'boke_2-recent',                                   // $this->id_base
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'boke_2_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'boke_2_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'boke_2_recent_widget_' . $this->id );
	}
