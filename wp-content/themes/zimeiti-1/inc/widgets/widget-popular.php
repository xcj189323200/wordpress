 ...
 * @since      1.0.0
 */
class zimeiti_1_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-zimeiti_1-popular widget_posts_thumbnail',
			'description' => __( '显示评论最多的文章（带缩略图）', 'zimeiti-1' )
		);

		// Create the widget.
		parent::__construct(
			'zimeiti_1-popular',                                   // $this->id_base
			__( '&raquo; 评论最多的文章', 'zimeiti-1' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'zimeiti_1_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'zimeiti_1_popular_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'zimeiti_1_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( '评论最多的文章', 'zimeiti-1' ),
			'limit'     => 5,
			'show_views' => true
		);
