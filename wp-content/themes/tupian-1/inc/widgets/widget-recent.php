 ...
 * @since      1.0.0
 */
class tupian_1_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-tupian_1-recent widget_posts_thumbnail',
			'description' => __( '显示最新的文章（带缩略图）', 'boke-2' )
		);

		// Create the widget.
		parent::__construct(
			'tupian_1-recent',                                   // $this->id_base
			__( '&raquo; 最新的文章', 'boke-2' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'tupian_1_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'tupian_1_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'tupian_1_recent_widget_' . $this->id );
	}
