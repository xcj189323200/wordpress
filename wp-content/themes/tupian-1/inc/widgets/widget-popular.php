 ...
 * @since      1.0.0
 */
class tupian_1_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-tupian_1-popular widget_posts_thumbnail',
			'description' => __( '显示评论最多的文章（带缩略图）', 'boke-2' )
		);

		// Create the widget.
		parent::__construct(
			'tupian_1-popular',                                   // $this->id_base
			__( '&raquo; 评论最多的文章', 'boke-2' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'tupian_1_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'tupian_1_popular_widget_' . $this->id, $popular );
				...
				while ( $popular->have_posts() ) : $popular->the_post();
					...
					echo '<li class="clear">';
						if ( has_post_thumbnail() ) {
							...
							if( get_theme_mod('timthumb-on', false) == true ) {
								echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . '<div class="thumbnail-wrap">';
	...
	function flush_widget_transient() {
		delete_transient( 'tupian_1_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( '评论最多的文章', 'boke-2' ),
			'limit'     => 5,
			'show_views' => true
		);
