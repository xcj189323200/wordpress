...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( '喜欢这篇文章', 'boke-2' ),
		'unlike' => __( '取消喜欢', 'boke-2' )
	) ); 
}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( '取消喜欢', 'boke-2' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( '喜欢这篇文章', 'boke-2' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	...
	$count = '<span class="sl-count"><i class="fa fa-thumbs-o-up"></i> ' . esc_html('喜欢(', 'boke-2') . $number . ')</span>';
	return $count;
} // get_like_count()
