 ...
 * @since      1.0.0
 */
class zazhi_1_Block_Three_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-zazhi-1-home-three-columns',
			'description' => __( '只能添加到“首页内容(栏目)”', 'zazhi-1' )
		);

		// Create the widget.
		parent::__construct(
			'zhutibaba-home-three-columns',         // $this->id_base
			__( '&raquo; 首页内容 - 3个栏目', 'zazhi-1' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			echo '<div class="content-block content-block-3 clear">';
				...
				zazhi_1_home_posts_left_col( $args, $instance );

				zazhi_1_home_posts_middle_col( $args, $instance );

				zazhi_1_home_posts_right_col( $args, $instance );				

				echo '</div><!-- .blocks-wrap -->';
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cat_3' ); ?>"><?php _e( '选择一个分类目录（第三列）', 'zazhi-1' ); ?></label>
 ...
 * Column left posts
 */
function zazhi_1_home_posts_left_col( $args, $instance ) {
 ...
 * Column middle posts
 */
function zazhi_1_home_posts_middle_col( $args, $instance ) {
 ...
 * Column right posts
 */
function zazhi_1_home_posts_right_col( $args, $instance ) {
